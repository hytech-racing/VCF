...
SteeringParams_s gen_default_params(){
    ...
    params.min_steering_signal_analog = 25;
...
TEST(SteeringSystemTesting, test_adc_to_degree_conversion)
{
    ...
    EXPECT_NEAR(data.analog_steering_angle, 0.0f, 0.001f); //fail
    EXPECT_NEAR(data.digital_steering_angle, 0.0f, 0.001f); //fail
    ...
    EXPECT_NEAR(data.analog_steering_angle, expected_analog_min, 0.001f); //fail
    EXPECT_NEAR(data.digital_steering_angle, expected_digital_min, 0.001f); //fail
    ...
    EXPECT_NEAR(data.analog_steering_angle, expected_analog_max, 0.001f); //fail
    EXPECT_NEAR(data.digital_steering_angle, expected_digital_max, 0.001f); //fail
...
TEST(SteeringSystemTesting,test_sensor_output_logic){
    ...
    steering.evaluate_steering(both_valid_disagree, 1010);
    data = steering.get_steering_system_data();
    ...
    steering.evaluate_steering(digital_bad, 1020);
    data = steering.get_steering_system_data();
    EXPECT_TRUE(data.digital_oor_implausibility);
    EXPECT_FALSE(data.analog_oor_implausibility); //actual true, expected false 
    ...
    steering.evaluate_steering(analog_bad, 1030);
    data = steering.get_steering_system_data();
    EXPECT_TRUE(data.analog_oor_implausibility);
    EXPECT_FALSE(data.digital_oor_implausibility); //actual true, expected false
    ...
    steering.evaluate_steering(both_bad, 1040);
    data = steering.get_steering_system_data();
    EXPECT_TRUE(data.analog_oor_implausibility);
    EXPECT_TRUE(data.digital_oor_implausibility);

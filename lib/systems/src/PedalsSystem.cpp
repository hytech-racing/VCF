...
PedalsSystemData_s PedalsSystem::evaluate_pedals(PedalSensorData_s pedals_data, unsigned long curr_millis)
{
    ...
    out.brake_is_implausible = _evaluate_pedal_implausibilities(brake_1,brake_2,_brakeParams,1.0); // 1.0 is a placeholder here because we don't need this for brake, please review
...
bool PedalsSystem::_evaluate_pedal_implausibilities(int pedal_data1_analog, int pedal_data2_analog, const PedalsParams &params, float max_percent_diff){
    ...
    float pedal1_scaled = abs(static_cast<float>(pedal_data1_analog) - std::min(params.min_pedal_1,params.max_pedal_1)) / abs(params.max_pedal_1 - params.min_pedal_1);
    float pedal2_scaled = abs(static_cast<float>(pedal_data2_analog) - std::min(params.min_pedal_2,params.max_pedal_2)) / abs(params.max_pedal_2 - params.min_pedal_2);

...
bool SteeringSystem::_evaluate_steering_oor_analog(uint32_t steering_analog_raw) // RAW
        {
            ...
            const uint32_t analog_margin_counts = static_cast<uint32_t>(_steeringParams.analog_tol * _steeringParams.span_signal_analog);
            ...
            return (steering_analog_raw < min_ok || steering_analog_raw > max_ok);
        }
...
bool SteeringSystem::_evaluate_steering_dtheta_exceeded(float dtheta){
           return (abs(dtheta) > _steeringParams.max_dtheta_threshold);
        }
...
float SteeringSystem::_convert_digital_sensor(const SteeringSensorData_s &current_steering_data)
{
    ...
    const uint32_t curr_raw_digital = current_steering_data.digital_steering_analog; //adc value
    //create a span for the digital sensor
    const uint32_t span_digital = _steeringParams.max_steering_signal_digital - _steeringParams.min_steering_signal_digital;
...
SteeringSystemData_s SteeringSystem::evaluate_steering(const SteeringSensorData_s &current_steering_data, const uint32_t current_millis){
    ...
    if(!_first_run && dt > 0){ //check that we not on the first run which would mean no previous data
        float dtheta_analog = curr_analog_deg - _prev_analog_angle; //prev_angle established in last run
        float dtheta_digital = curr_digital_deg - _prev_digital_angle;
        ...
        const float allowed_difference = analog_tol + _steeringParams.digital_tol_deg; //the allowed difference equals digital error+ analog error

...
void SteeringSystem::recalibrate_steering_digital(const SteeringSensorData_s &current_steering_data, bool calibration_is_on){
    ...
    _steeringParams.deg_per_count_digital = (180.0f)/(_steeringParams.max_steering_signal_digital - _steeringParams.min_steering_signal_digital); //assigns value for deg_per_ct
}

bool SteeringSystem::_evaluate_steering_oor_analog(uint32_t steering_analog_deg) // CONVERTED
        {
            ...
            return (steering_analog_deg < min_ok || steering_analog_deg > max_ok);
        }

bool SteeringSystem::_evaluate_steering_oor_digital(uint32_t steering_digital_deg) // CONVERTED
        {
        //check if the digital value is within bounds +- error
        
            // const uint32_t digital_margin_counts = static_cast<uint32_t>(_steeringParams.digital_tol_deg /_steeringParams.deg_per_count_digital);
            const uint32_t min_ok = _steeringParams.min_steering_signal_digital - _steeringParams.digital_tol_deg;
            const uint32_t max_ok = _steeringParams.max_steering_signal_digital + _steeringParams.digital_tol_deg;
            return (steering_digital_deg < min_ok || steering_digital_deg > max_ok);
        }
...
{
    // Get the raw value
    const uint32_t curr_raw_analog = current_steering_data.analog_steering_degrees; //is this adc input value?
    ...
    const float curr_analog_degrees = curr_analog_offset * _steeringParams.deg_per_count_analog;
...
float SteeringSystem::_convert_digital_sensor(const SteeringSensorData_s &current_steering_data)
{
    ...
    const uint32_t span_digital = static_cast<uint32_t>(_steeringParams.max_steering_signal_digital) - static_cast<uint32_t>(_steeringParams.min_steering_signal_digital);

    //degree span we are assuming:
    constexpr float kSpan = 180.0f;
    _steeringParams.deg_per_count_digital = kSpan/(float)span_digital;

    const uint32_t digital_midpoint = ((_steeringParams.max_steering_signal_digital+_steeringParams.min_steering_signal_digital)/2.0f)*_steeringParams.deg_per_count_digital;

    const int32_t count_delta = (int32_t)raw_val - (int32_t)digital_midpoint;

    float degrees_delta = (float)count_delta * _steeringParams.deg_per_count_digital;
    //output in degreees_delta
    return digital_midpoint + degrees_delta;
}
...
SteeringSystemData_s SteeringSystem::evaluate_steering(const SteeringSensorData_s &current_steering_data, uint32_t current_millis){
    ...
    const uint32_t curr_analog_deg = _convert_analog_sensor(current_steering_data);
    const uint32_t curr_digital_deg = _convert_digital_sensor(current_steering_data);
    //Conversion from raw ADC to degrees
    out.analog_steering_angle = curr_analog_deg;
    out.digital_steering_angle = curr_digital_deg;
    ...
    if(!_first_run && dt > 0){ //check that we not on the first run which would mean no previous data
        float dtheta_analog = fabsf(out.analog_steering_angle - _prev_analog_angle); //prev_angle established in last run
        ...
        out.analog_oor_implausibility = _evaluate_steering_oor_analog(curr_analog_deg);
        out.digital_oor_implausibility = _evaluate_steering_oor_digital(curr_digital_deg);
        ...
        const float analog_tol_deg = _steeringParams.analog_tol_deg*full_scale_deg_analog*_steeringParams.deg_per_count_analog; // analog percentage*adc span *deg/adc count
        const float allowed_difference = analog_tol_deg+ _steeringParams.digital_tol_deg; //the allowed difference equals digital error+ analog error
        ...
        if (analog_valid && digital_valid) 
        {
            ...
            {
                out.output_steering_angle = out.analog_steering_angle;//default to original, but we need to consider what we really want to put here
            }
        }
